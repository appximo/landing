...
PAGE_CSS='''<style>
  ...
  .bulb .bulb-ray{stroke:var(--color-brand-400)}
  /* el bombillo se anima siempre (una pasada de 2 s: trazo → destello → rayos); no es un bucle */
  @keyframes bulbDraw{from{stroke-dashoffset:60;opacity:.35}to{stroke-dashoffset:0;opacity:1}}
  @keyframes bulbOn{0%{opacity:0;transform:scale(.4)}60%{opacity:1;transform:scale(1.25)}100%{opacity:1;transform:scale(1)}}
  @keyframes bulbFlash{0%{opacity:0;transform:scale(.3)}40%{opacity:.9;transform:scale(1.6)}100%{opacity:0;transform:scale(2.4)}}
  @keyframes bulbGlow{0%{filter:drop-shadow(0 0 0 transparent)}50%{filter:drop-shadow(0 0 14px color-mix(in srgb,var(--color-brand) 90%,transparent))}100%{filter:drop-shadow(0 0 5px color-mix(in srgb,var(--color-brand) 60%,transparent))}}
  .bulb{position:relative}
  .bulb::before{content:"";position:absolute;inset:0;border-radius:50%;background:radial-gradient(circle,color-mix(in srgb,var(--color-brand) 70%,transparent),transparent 65%);opacity:0;pointer-events:none}
  html.js .bulb .bulb-glass{stroke-dasharray:60;animation:bulbDraw 1.1s var(--ease) .3s both}
  html.js .bulb .bulb-ray{transform-origin:center;transform-box:fill-box;opacity:0;animation:bulbOn .55s cubic-bezier(.34,1.56,.64,1) 1.45s both}
  html.js .bulb::before{animation:bulbFlash .9s ease-out 1.35s both}
  html.js .bulb{animation:bulbGlow 1.3s var(--ease) 1.4s both}
  html.js .bulb.lit .bulb-glass{stroke:var(--color-brand-300)}
  ...
  .stage{position:relative;display:none;margin-left:auto;max-width:25rem;padding:1.1rem;border-radius:var(--radius-3xl);background:var(--white);color:var(--text);border:1px solid rgba(255,255,255,.35);box-shadow:0 40px 90px -30px rgba(0,0,0,.7),0 0 0 8px rgba(255,255,255,.04)}
  .stage::before{content:"";position:absolute;inset:-14%;z-index:-1;background:radial-gradient(closest-side,rgba(255,255,255,.10),transparent 70%);pointer-events:none}
  .stage .fl-k{color:var(--color-brand-ink)}
  .stage .fl-bubble{background:var(--zinc-100);border-color:var(--zinc-200);color:var(--text);font-weight:500}
  .stage .fl-arrow{color:var(--color-brand-ink)}
  .stage .fl-arrow::before{background:repeating-linear-gradient(180deg,var(--color-brand-500) 0 4px,transparent 4px 8px)}
  .stage .plat{box-shadow:var(--shadow-soft);border:1px solid var(--zinc-200)}
  .stage .k{background:linear-gradient(180deg,transparent 64%,var(--color-brand-200) 64%)}
  .stage .fl-chips .chip-ghost{border-color:var(--zinc-200);background:var(--zinc-50);color:var(--zinc-700)}
  ...
  .plat-kpis b span{display:inline;font-size:inherit;color:inherit;line-height:inherit}
  ...
  /* narrativa de UNA pasada: corre siempre (no es un bucle); solo el parpadeo del cursor respeta reducir-movimiento */
  ...
  @media (prefers-reduced-motion:no-preference){html.js .flow.typing .fl-caret{animation:caret .9s steps(1) infinite}}
  ...
  html.js .flow.typed .fl-bubble{transform:scale(.97);opacity:.8}
  ...
  /* ── el escenario blanco de «Cómo funciona» ── */
  .build{background:var(--white);color:var(--text);border-radius:var(--radius-2xl);box-shadow:var(--shadow-lift);padding:1rem 1.1rem 1.1rem;position:relative;overflow:hidden}
  .b-head{display:flex;justify-content:space-between;align-items:center;margin-bottom:.9rem}
  .b-time{font-size:.78rem;color:var(--text-3);font-weight:600}
  .b-chat{display:flex;gap:.6rem;align-items:flex-start}
  .b-who{font-size:.68rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--text-3);padding-top:.5rem;flex:none}
  .b-bubble{background:var(--zinc-100);border-radius:var(--radius-xl) var(--radius-xl) var(--radius-xl) .3rem;padding:.7rem .9rem;font-size:.92rem;line-height:1.45;color:var(--text);min-height:2.6rem;flex:1}
  .b-caret{display:none;width:2px;height:1em;background:var(--color-brand-ink);vertical-align:-.15em;margin-left:2px}
  .b-proc{list-style:none;margin:.9rem 0 0;display:grid;gap:.45rem;padding-left:0}
  .b-proc li{display:flex;align-items:center;gap:.6rem;font-size:.86rem;color:var(--text-3)}
  .b-proc li i{width:1rem;height:1rem;border-radius:50%;border:2px solid var(--zinc-300);flex:none;position:relative;transition:border-color .3s,background .3s}
  .b-proc li.on{color:var(--text)}
  .b-proc li.on i{border-color:var(--color-brand-500)}
  .b-proc li.done{color:var(--text-2)}
  .b-proc li.done i{background:var(--color-brand-500);border-color:var(--color-brand-500)}
  .b-proc li.done i::after{content:"";position:absolute;left:.28rem;top:.1rem;width:.28rem;height:.5rem;border:solid var(--color-on-brand);border-width:0 2px 2px 0;transform:rotate(45deg)}
  .b-out{margin-top:.9rem;border-radius:14px;overflow:hidden;border:1px solid var(--zinc-200);box-shadow:var(--shadow-soft)}
  .b-out .bbar{background:var(--color-ink-800)}
  .b-plat{border-radius:0;box-shadow:none}
  .b-order{display:flex;align-items:center;justify-content:space-between;gap:.6rem}
  .b-plat::after{border-radius:0}
  html.js .build:not(.p3) .b-out{display:none}
  html.js .build:not(.p2) .b-proc{opacity:.45}
  @keyframes procPulse{0%,100%{box-shadow:0 0 0 0 color-mix(in srgb,var(--color-brand) 45%,transparent)}70%{box-shadow:0 0 0 .5rem transparent}}
  html.js .build.p1 .b-caret{display:inline-block}
  @media (prefers-reduced-motion:no-preference){html.js .build.p1 .b-caret{animation:caret .9s steps(1) infinite}}
  html.js .b-proc li.on i{animation:procPulse 1s ease-out 3}
  html.js .build.p3 .b-out{animation:platIn .7s var(--ease) both}
  html.js .build.p3 .b-plat::after{animation:sweep 1.5s var(--ease) .3s 1 both}
  /* ── la franja de demos ── */
    ...
    <div>
      <div class="build" id="build" aria-hidden="true">
        ...
        <ol class="b-proc">
          ...
          <li><i></i><span>Creando las pantallas y los avisos</span></li>
          <li><i></i><span>Publicando en internet, con su clave</span></li>
        </ol>
        <div class="b-out">
          ...
          <div class="plat b-plat">
            <div class="plat-tabs"><span class="on" data-k="pedidos">Pedidos</span><span data-k="clientes">Clientes</span><span data-k="inventario">Inventario</span><span data-k="cuentas">Cuentas</span></div>
            <div class="plat-kpis"><div><b class="tnum">$$&nbsp;<span class="late-count" data-count="2379570" data-dec="0">2.379.570</span></b><span>ventas confirmadas</span></div><div><b class="tnum late-count" data-count="13" data-dec="0">13</b><span>pedidos</span></div><div><b>IVA</b><span>listo para el contador</span></div></div>
            <div class="co-order b-order"><span><b style="font-size:.95rem;display:block">Martillo de uña 16 oz</b><span class="t-xs muted">2 unidades · recoge en tienda</span></span><span class="chip chip-brand"><i class="dot"></i>Pago confirmado</span></div>
          </div>
        </div>
      </div>
